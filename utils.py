...
import numpy as np
from data_utils import  ComposedVideoDataset
...
def extract_vdo_features(ref_vdo, clip_model, clip_preprocess):
    vr = VideoReader(ref_vdo)
    raw_sample_frms = vr.get_batch(list(range(0, len(vr), 8))).asnumpy()
    images = torch.stack([clip_preprocess(Image.fromarray(fr)) for fr in raw_sample_frms])
...
def extract_index_features(dataset: Union[ComposedVideoDataset], clip_model: CLIP) -> \
    ...
    index_features = torch.empty((0, 8, feature_dim)).to(device, non_blocking=True)
    index_names = []
    ...
    if isinstance(dataset, ComposedVideoDataset):
        print(f"extracting ComposedVideoDataset index features")
    for names, vdo_fea in tqdm(classic_val_loader):
        vdo_fea = vdo_fea.to(device, non_blocking=True)
        # images = images.to(device, non_blocking=True)
        # with torch.no_grad():
        #     batch_features = clip_model.encode_image(images)
        index_features = torch.vstack((index_features, vdo_fea))
        index_names.extend(names.numpy())
...
def set_train_bar_description(train_bar, epoch: int, num_epochs: int, train_running_results: dict):
    ...
    train_bar.set_description(
        desc=f"[{epoch}/{num_epochs}] "
             ...
             f"similar percentage: {train_running_results['similar_per']:.3f} "
    )

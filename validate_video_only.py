    ...
    with torch.no_grad():
        ...
        for bt_index in range(int(len(index_features) / b) + 1):
            if bt_index < int(len(index_features) / b):
                index_features_tmp.append(index_features[bt_index * b:(bt_index + 1) * b, ].mean(1))

            else:
                index_features_tmp.append(index_features[bt_index * b:, ].mean(1))
...
def generate_val_predictions(clip_model: CLIP, relative_val_dataset: ComposedVideoDataset,
    ...
    for batch_reference_names, batch_target_names, (captions, captions_wo_negation), batch_group_members, middle_feature in tqdm(
        ...
        with torch.no_grad():
            ...
            batch_predicted_features = reference_image_features.mean(1)
...
def main():
    ...
    parser.add_argument("--combining-function", type=str, default='sum',
